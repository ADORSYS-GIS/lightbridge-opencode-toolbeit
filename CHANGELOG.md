...
- **oauth2, models-info, ratelimit, browser, browser-mcp, devtools, devtools-mcp, otel, code-index:** Switched the `build` script's JS-emission engine from `tsc` to SWC (`@swc/core` + `@swc/cli`, latest as of this change) across all nine library packages. `tsc -p tsconfig.json --emitDeclarationOnly` still runs immediately after to emit `.d.ts` — SWC does not emit declarations, and these are published packages whose `types` / `./lib` exports depend on them. `typecheck` (`tsc --noEmit`) is unchanged and remains the type-safety gate. A single root `.swcrc` (ES2022, ESM, source maps, `.d.ts` excluded from compilation) is shared by all nine packages via `../../.swcrc`, mirroring the existing root `tsconfig.base.json` pattern. Verified byte-identical `dist/` file lists (198 files, same nesting) against the prior all-`tsc` build. See [ADR-0010](docs/adr/0010-swc-for-js-emit-tsc-for-declarations.md) — notably, this does **not** speed up `pnpm -r build` for this codebase (measured ~2.43s → ~2.77s workspace-wide, ~0.90s → ~0.98s for a single package), because declaration emission still requires a full `tsc` type-check, so the SWC pass is additive rather than substitutive. `plugin-bundle` (Rolldown) and `apps/browser-extension` (WXT) are unaffected.
