...
function createOpenCodeLogger(client: PluginInput["client"], getMinLevel: () => LogLevel): Logger {
  ...
  // warn/error about the caller's own throttled request stays off the
  // terminal and goes only to the host log, so only mirror to the JSON
  // console when explicitly requested; set VYMALO_PLUGIN_CONSOLE_LOG=1 to
  // restore full console output.
